...
const SessionPaymentPage = () => {
  ...
  const { sessionId, tutor, selectedTime, selectedDuration, subject, total } = location.state || {};
  const [paymentMethod, setPaymentMethod] = useState('credit_card');

  // Validation for missing session data or user
  ...
  return (
    <div className="max-w-xl mx-auto p-6">
      ...
      <div className="bg-white shadow-lg rounded-lg p-6 space-y-4">
        ...
        >
          {loading ? 'Processing...' : `Pay ${total} ETB`}
        </button>
      </div>
    </div>
  );
